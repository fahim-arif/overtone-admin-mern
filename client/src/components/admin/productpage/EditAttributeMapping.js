...
class EditAttributeMapping extends Component {
  constructor() {
    super();
    this.state = {
      _id: '',
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.attributemapping.editattributemapping !== this.props.attributemapping.editattributemapping) {
      Toast.fire({
        ...
        this.props.history.push(`addmapping?productID=614d7297a9c0ba3aeec9e672`)
    ...
    const dependentField = this.state.dependentField.map((value, index) => {
      ...
      return <React.Fragment>
        <div className="row border py-2 mt-4">
          <div style={{ minWidth: '180px' }} class="col-md-2">
            <label className="col-form-label main_title">Type </label>
            <select style={{ border: '1px solid #000' }} name="type" onChange={(e) => this.onhandleChangeField(e, index)} value={value.type} className="form-control" placeholder="" >
              <option value="">Select type</option>
              <option value="dropdown">Dropdown</option>
              <option value="color">Color Code</option>
            </select>
            <label className="col-form-label main_title mt-3">Label </label>
            <input style={{ border: '1px solid #000' }} type="text" name="label" onChange={(e) => this.onhandleChangeField(e, index)} value={value.label} className="form-control" placeholder="" />
          </div>
          <div style={{ maxWidth: '350px', border: '1px solid #000!important' }} class="col-md-8 border border-success py-2">

            {list.map((result, indexSub) => {
              return <div class="mt-3">
                <label className="col-lg-1 col-form-label main_title">Label:</label>
                <div className="col-lg-2">
                  <input style={{ minWidth: '305px', border: '1px solid #000' }} type="text" required name="label" onChange={(e) => this.onhandleChangeSubField(e, index, indexSub)} value={result.label} className="form-control" placeholder="" />
                </div>
                <label className="col-lg-1 col-form-label main_title">Value:</label>
                <div className="col-lg-2">
                  <input style={{ minWidth: '305px', border: '1px solid #000' }} type="text" required name="value" onChange={(e) => this.onhandleChangeSubField(e, index, indexSub)} value={result.value} className="form-control" placeholder="" />
                </div>
                <label className="col-lg-1 col-form-label main_title">Price:</label>
                <div className="col-lg-2">
                  <input style={{ minWidth: '305px', border: '1px solid #000' }} type="text" required name="additionalPrice" onChange={(e) => this.onhandleChangeSubField(e, index, indexSub)} value={result.additionalPrice} className="form-control" placeholder="" />
                </div>
                <div className="col-lg-3 " style={{ display: 'flex' }}>
                  <button type="button" value={value.type} onClick={() => this.addSubField(index)} className="btn btn-success btn-sm mt-3">
                    Add
                    {/* <i class="fa fa-plus"></i> */}

                  </button>
                  &nbsp;&nbsp;&nbsp;
                  {list.length > 1 ? <button type="button" value={value.type} onClick={() => this.removeSubField(index, indexSub, value.label)} className="btn btn-danger btn-sm mt-3"><i class="fa fa-trash"></i></button> : null}
                </div>
              </div>
            })}
          </div>
          <div style={{ alignItems: 'center', flexDirection: 'column', justifyContent: 'center' }} className="col-lg-2 d-flex align-content-center">
            <button style={{ minWidth: '90px', maxHeight: '28px' }} type="button" value={value.type} onClick={() => this.addField()} className="btn btn-success btn-sm mt-1">
              Add
              {/* <i class="fa fa-plus"></i> */}
            </button>
            &nbsp;&nbsp;&nbsp;
            {this.state.dependentField.length > 1 ? <button style={{ minWidth: '90px' }} type="button" value={value.type} onClick={() => this.removeField(index, value.title)} className="btn btn-danger btn-sm mt-1"><i class="fa fa-trash"></i></button> : null}
          </div>
        </div>
      </React.Fragment>
    })
...
EditAttributeMapping.propTypes = {
...
export default connect(mapStateToProps, { editAttributeMapping, listParentAttributeCategory, listAttributeCategory, listProduct })(EditAttributeMapping);