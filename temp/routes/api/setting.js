...
router.get('/home',async (req,res) => {
  ...
  //Get Category ID
  var catID= await Category.findOne({categoryName:/^MARKETPLACE$/i});
  console.log("catID",catID)
   ...
   footerContent2= await  SubCategory.find({$and:[{isEnabled:'Yes'},{ categoryID: { $nin: [catID._id] } }]}).sort({date:-1}).limit(4);
    res.json({
