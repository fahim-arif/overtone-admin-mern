...
router.get("/test", async (req, res) => {
  AttributeMapping.aggregate([
    {
      $match: {
        productID: mongoose.Types.ObjectId("60be395a52dc3318440512b4"),
      },
    },
    { $sort: { date: -1 } },
    {
...
router.post("/web/getshop", async (req, res) => {
  ...
  var OtherCategoryView = await SubCategory.aggregate([
    {
      $match: {
        categoryID: { $nin: [mongoose.Types.ObjectId(req.body.categoryID)] },
      },
    },
    {
      $lookup: {
...
router.post("/web", (req, res) => {
  ...
  if (req.body.categoryID) {
    // if the criteria has a value or
    q["$and"].push({
  ...
  if (req.body.subcategoryChildID) {
    // if the criteria has a value or
    q["$and"].push({
      subcategoryChildID: mongoose.Types.ObjectId(req.body.subcategoryChildID),
    }); // add to the query object
  }
...
router.post("/maxstock", (req, res) => {
  Product.findOne({ _id: req.body.productID }).then((productData) => {
    var variationArray = productData.variationArray
      ? JSON.parse(productData.variationArray)
      : [];
    var stockCheck = variationArray.find(
      (x) =>
        x.size === req.body.selectedSize && x.color === req.body.selectedColor
    );
    if (stockCheck) {
...
router.post(
  ...
  (req, res) => {
    ...
    if (req.user.userType === "store") {
      ...
      Product.findOne({
        $and: [{ storeID: req.user.id }, { name: req.body.name }],
      }).then((result) => {
        if (result) {
          errors.productName = "Product Name Already Exists";
          return res.status(404).json(errors);
